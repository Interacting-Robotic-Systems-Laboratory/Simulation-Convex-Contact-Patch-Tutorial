...
LHS(4:6,:)=Tensor_I*(W-W_old)+h*cross(W_old,Tensor_I*W_old);
...
syms x y z real;
gcn = z;
%% contact constraints for cube

syms len wid heg real; % dimension of the cuboid
 
syms l1 l2 l3 l4 l5 l6 l7 real; % lagrangue multiplier
...
DST(1:3,1)=(a2-a1)+l7*(grad_a2);
DST(4:6,1)=grad_a2+sum_grad_a1;
DST(7:12,1) = -subs(Fcn,{x,y,z},{a1_x,a1_y,a1_z});
DST(13,1) = -subs(gcn,{x,y,z},{a2_x,a2_y,a2_z});  
DST(14,1) = subs(gcn,{x,y,z},{a1_x,a1_y,a1_z});

DST = simplify(DST);
fcn = [DYM;P_friction(1:3);DST(1:6);P_friction(4);DST(7:14)];
fcn = simplify(fcn);
...
T(1,1:24) = Z;
T(1,25:33) = [R11 R12 R13 R21 R22 R23 R31 R32 R33];
T(1,34:42) = [I11 I12 I13 I21 I22 I23 I31 I32 I33];
T(1,43:45) = [q_x q_y q_z];
J1 = jacobian(fcn',T);
